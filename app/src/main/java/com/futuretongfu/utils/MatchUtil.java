...
public class MatchUtil {
    ...
    /**
     *   1) 密码控制只能输入字母、数字、不包含特殊符号(~!@#$%^&*()_+[]{}|\;:'",./<>?)
     *   2) 长度 6-16 位，必须包括字母、数字、特殊符号中的2种
     * */
    public static boolean matchPassowrd(String password, int pswMinLength, int pswMaxLength){
        ...
        //判断密码是否包含特殊符号(~!@#$%^&*()_+|<>,.?/:;'[]{}\)：包含返回true，不包含返回false
        boolean k = password.matches(".*[~!@#$%^&*()_+|<>,.?/:;'\\[\\]{}\"]+.*") ? true : false;
        if(k){
            return false;
        }
        ...
        if (i + j <= 1 || length < pswMinLength || length > pswMaxLength) {
            return false;
        }
    ...
    public static boolean isName(String str) {
        ...
        p = Pattern.compile("/(^[\\u4e00-\\u9fa5]+$)|(^[\\u4e00-\\u9fa5]+(\\·?[\\u4e00-\\u9fa5]+)+$)|(^[a-zA-Z]+[\\s.]?([a-zA-Z]+[\\s.]?){0,4}[a-zA-Z]$)/");
        m = p.matcher(str);
        b = m.matches();
    ...
    public static boolean isHasSpecialSymbol(String text){
        return text.matches(".*[~!@#$%^&*()_+|<>,.?/:;'\\[\\]{}\"]+.*") ? true : false;
    }

    /**
     * 昵称 验证字符是否包含特殊符号  可包含_
     *
     */
...
//        //判断是否包含数字：包含返回1，不包含返回0
//        int i = text.matches(".*\\d+.*") ? 1 : 0;
//
//        if(1 == i){
//            //判断是否包含字母：包含返回1，不包含返回0
//            int j = text.matches(".*[a-zA-Z]+.*") ? 1 : 0;
//
//            if(0 == j){
//                if(isHasSpecialSymbol(text))
//                    return false;
//
//                return true;
//            }
...
//        return false;
    ...
    private static boolean validate18Idcard(String idcard) {
        ...
        if (null == checkCode) {
            //Logger.d(Tag, "将和值与11取模得到余数进行校验码判断");
            return false;
        }
        // 将身份证的第18位与算出来的校码进行匹配，不相等就为假
        if (!idcard18Code.equalsIgnoreCase(checkCode)) {
            //Logger.d(Tag, "将身份证的第18位与算出来的校码进行匹配，不相等就为假");
            return false;
        }
    ...
    public static String formatFloatNumber(Double value) {
        return StringUtil.getDouble2(value);
//        if(value != null){
