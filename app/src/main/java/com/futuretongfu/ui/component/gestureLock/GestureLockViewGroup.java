...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    //两个GestureLockView间连线宽度
    private int mGestureLockLineWidth;
    ...
    private int mFingerUpColorErr = 0xFF4848;
                                ...
                                int defStyle)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            switch (attr)
            {
                ...
                case R.styleable.GestureLockViewGroup_color_finger_up:
                    ...
                    break;
                case R.styleable.GestureLockViewGroup_color_finger_up_err:
                    mFingerUpColorErr = a.getColor(attr, mFingerUpColorErr);
                    break;
                ...
                case R.styleable.GestureLockViewGroup_tryTimes:
                    ...
                    break;
                case R.styleable.GestureLockViewGroup_lock_margin:
                    mMarginBetweenLockView = (int)a.getDimension(attr, 15F);
                    break;
                case R.styleable.GestureLockViewGroup_lock_width:
                    mGestureLockViewWidth = (int)a.getDimension(attr, 60F);
                    break;
                case R.styleable.GestureLockViewGroup_line_width:
                    mGestureLockLineWidth = (int)a.getDimension(attr, 2F);
                    break;
                case R.styleable.GestureLockViewGroup_innerCircleRadius:
                    mInnerCircleRadius = a.getDimension(attr, 20);
                    break;
                default:
                    break;
            }
        ...
        mPaint.setStrokeWidth(mGestureLockLineWidth);
        ...
        if (mGestureLockViews == null)
        {
            ...
            for (int i = 0; i < mGestureLockViews.length; i++)
            {
                ...
                mGestureLockViews[i] = new GestureLockView(getContext(),
                        ...
                        mFingerOnColor, mFingerUpColor, mFingerUpColorErr, mInnerCircleRadius);
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        switch (action)
        {
            ...
            case MotionEvent.ACTION_UP:
                ...
                {
                    if(!isCheckPassword){
                        mOnGestureLockViewListener.onGestureFinish(mChoose);
                    }
                    else {
    ...
    private void changeItemMode() {
        ...
        if(mOnGestureLockViewListener != null){
            if(mOnGestureLockViewListener.onGestureError()){

                for (GestureLockView gestureLockView : mGestureLockViews) {
                    if (mChoose.contains(gestureLockView.getId())) {
                        gestureLockView.setMode(GestureLockView.Mode.STATUS_FINGER_UP_ERR);
                    }
                }
    ...
    private void reset()
    {
        ...
        if(mOnGestureLockViewListener != null)
            mOnGestureLockViewListener.onGestureReset();
    }
    /**
     * 检查用户绘制的手势是否正确
     * @return
    ...
    public void resetAndUpdateView(){
        reset();
        invalidate();
    }
    ...
    public void dispatchDraw(Canvas canvas)
    {
        ...
        if (mPath != null)
        {
            if(mGestureLockViews[0].isErrMode()){
                mPaint.setColor(mFingerUpColorErr);
            }
